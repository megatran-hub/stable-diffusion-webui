...
class Embedding:
    ...
    def save(self, filename):
        # Prepare the data dict for the embedding itself:
        embedding_data = {
            "string_to_token": {"*": 265},
        ...
        # If saving optimizer state is enabled and we have an optimizer saved:
        if shared.opts.save_optimizer_state and self.optimizer_state_dict is not None:
            optimizer_saved_dict = {
                'hash': self.checksum(),
                'optimizer_state_dict': self.optimizer_state_dict,
            }
            # Save the optimizer state separately:
            torch.save(
                optimizer_saved_dict,
                f"{filename}.optim",
...
class EmbeddingDatabase:
    ...
    def load_textual_inversion_embeddings(self, force_reload=False):
        ...
        if shared.opts.textual_inversion_print_at_load and self.previously_displayed_embeddings != displayed_embeddings:
            ...
            print(
                f"Textual inversion embeddings loaded({len(self.word_embeddings)}): "
                f"{', '.join(self.word_embeddings.keys())}"
            )
            if self.skipped_embeddings:
                print(
                    f"Textual inversion embeddings skipped({len(self.skipped_embeddings)}): "
                    f"{', '.join(self.skipped_embeddings.keys())}"
                )
...
def validate_train_inputs(model_name, learn_rate, batch_size, gradient_step, data_root,
                          template_file, template_filename, steps, save_model_every, create_image_every,
                          log_directory, name="embedding"):
...
def train_embedding(
    ...
    # n steps = batch_size * gradient_step * n images processed
    ...
    try:
        ...
        for _ in range((steps - initial_step) * gradient_step):
            ...
            for j, batch in enumerate(dl):
                ...
                description = (
                    f"Training textual inversion [Epoch {epoch_num}: "
                    f"{epoch_step + 1}/{steps_per_epoch}] loss: {loss_step:.7f}"
                )
                ...
                if embedding_dir is not None and steps_done % save_embedding_every == 0:
                    ...
                    save_embedding(
                        embedding, optimizer, checkpoint,
                        embedding_name_every, last_saved_file,
                ...
                if images_dir is not None and steps_done % create_image_every == 0:
                    ...
                    if (
                        ...
                        last_saved_image_chunks = os.path.join(
                            images_embeds_dir,
                            f'{embedding_name}-{steps_done}.png'
                        )
